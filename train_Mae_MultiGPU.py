...
import models.MAE.models_mae as mae
...
def train():
    ...
    # SETTINGS
    project_name = 'CV-MAE-Base-original'  # Set the project name
    num_epochs = 1000
    lr = 1e-4
    weight_decay = 1e-5
    train_batch_size = 64
    ...
    log_path = './logs/MAE_Base_original/'
    ...
    # model = mae.__dict__['mae_vit_large_patch16']()
    model = mae.__dict__['mae_vit_base_patch16']()
    # model = mae.__dict__['mae_vit_tiny_patch16']()

    # checkpoint = torch.load("checkpoints/MAE/mae_pretrain_vit_base.pth")
    #
    ...
    for epoch in range(num_epochs):
        ...
        for i, (image, mask, text_description) in enumerate(pbar):
            ...
            # Set the image to device
            image = image.to(device)

            # Convert the image from (B, W, H, C) to (B, C, W, H)
            ...
            loss, pred, mask = model(image)  # Forward pass
