...
from models.MAE.mae_seg import UNETR2D_MAE
...
def train():
    ...
    # SETTINGS
    project_name = 'CV-MAE-Seg-Large-retrain'  # Set the project name
    num_epochs = 500
    lr = 1e-4
    weight_decay = 1e-5
    train_batch_size = 64
    ...
    log_path = './logs/MAE_Seg_Large_retrain/'
    ...
    # Load the model
    mae_model = mae.__dict__['mae_vit_large_patch16']()
    # mae_model = mae.__dict__['mae_vit_base_patch16']()
    # mae_model = mae.__dict__['mae_vit_tiny_patch16']()

    # pretrained_dict = "logs/MAE_Large/best_model.pt"
    # pretrained_dict = "logs/MAE_Tiny/best_model.pt"
    # pretrained_dict = "logs/MAE/best_model.pt"

    # mae_model = torch.load(pretrained_dict,weights_only=False)
    #
    ...
    for epoch in range(num_epochs):
        ...
        for i, (image, mask, text_description) in enumerate(pbar):
            ...
            seg_output = model(image)  # Forward pass
            loss = total_seg_loss(mask, seg_output)  # Compute the loss
